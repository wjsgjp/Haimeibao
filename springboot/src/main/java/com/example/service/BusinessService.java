...
public class BusinessService {
    ...
    public void updateById(Business business) {
        ...
        Business dbBusiness1 = selectByBusinessId(business.getId());
    ...
    public List<Business> selectAll(Business business) {
        ...
        if (currentUser.getRole().equals(RoleEnum.BUSINESS.name())) {
            business.setId(currentUser.getId());
        }
        List<Business> businesses = businessMapper.selectAll(business);
        for (Business b : businesses) {
           List<Comment> c =  commentService.selectByBusinessId(b.getId());
            ...
            b.setScore(star);
            List<Orders> orders = ordersMapper.selectByBusinessId(b.getId());
            int num = 0;
    ...
    public Business selectByUsername(String username) {
        ...
        return businessMapper.selectOne(params);
     ...
     * 根据ID查询
     */
    public Business selectById(Integer userId,Integer id) {
        Business params = new Business();
        params.setId(id);
        Business business = businessMapper.selectOne(params);
        Collect collect = collectService.selectByUserIdAndBusinessId(userId, id);
        if (ObjectUtil.isNotEmpty(collect)) {
            business.setIsCollect(true);
        }
        return business;
    ...
    public void updatePassword(Account account) {
        Business business = this.selectByBusinessId(account.getId());
    ...
    public void checkBusinessAuth() {
        ...
        if (RoleEnum.BUSINESS.name().equals(currentUser.getRole())) {   // 如果是商家 的话
            Business business = selectByBusinessId(currentUser.getId());
